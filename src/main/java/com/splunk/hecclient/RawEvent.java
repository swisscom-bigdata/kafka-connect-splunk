...
public final class RawEvent extends Event {
    ...
    public RawEvent(Object data, Object tied) {
        super(data, tied);
        setLineBreaker("");  // By default disable carriage return line breaker
    }
    ...
    public byte[] getBytes() {
        ...
        if (event instanceof String) {
            ...
            try {
                ...
                log.error("Failed to encode RawEvent in UTF-8", ex);
                throw new HecException("Failed to encode RawEvent in UTF-8", ex);
            }
        } else if (event instanceof byte[]) {
            bytes = (byte[]) event;
        } else { // Not String, Not Byte[], must be JSON object
            try {
                bytes = jsonMapper.writeValueAsBytes(event);
            } catch (Exception ex) {
                log.error("Invalid json data, Failed to serialize RawEvent to Byte[]", ex);
                throw new HecException("Failed to serialize RawEvent to Byte[]", ex);
            }
        }
        return bytes;
    }
    ...
    public String toString() {
        try {
            ...
            log.error("Failed to decode RawEvent as UTF-8", ex);
            throw new HecException("Failed to decode RawEvent as UTF-8", ex);
        }
    }
}
